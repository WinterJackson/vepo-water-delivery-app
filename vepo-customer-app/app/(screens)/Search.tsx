				...
				<View style={{ overflow: "hidden", paddingBottom: 4 }}>
					...
					>
						...
						<SearchBar
							...
							setFunc={setSearch}
						/>
						<PressableScale accessibilityLabel="Search" onPress={handleSearch} className={`w-[50px] h-[50px] items-center justify-center rounded-2xl ${darkTheme ? "bg-white/5" : "bg-white"}`}>
					...
					<View className="py-4">
						...
						<View className="flex-row flex-wrap gap-2">
							{history.map((h, i) => (
								<View 
									key={i} 
									className={`rounded-full flex-row items-center overflow-hidden ${darkTheme ? "bg-white/10" : "bg-white"}`}
								>
									<PressableScale
										onPress={() => {
											setSearch(h);
